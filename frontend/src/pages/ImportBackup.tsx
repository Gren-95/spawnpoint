...
export default function ImportBackup() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      setServers(servers);
      setTimeout(() => navigate(`/servers/${json.data.server.id}`), 1500);
    } catch (err: unknown) {
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <Archive size={24} className="text-mc-green" />
        <h1 className="text-2xl font-bold">Import from Backup</h1>
      </div>

      <p className="text-mc-muted text-sm mb-6">
        Import a <strong className="text-gray-300">.tar.gz</strong> full backup exported from Spawnpoint to create a new server with all its files restored.
        Server type and Minecraft version are auto-detected from the backup contents.
      </p>
        ...
        >
          ...
          {file ? (
            ...
            <div>
              ...
              <div className="text-xs text-mc-muted mt-1">Full backups only — world-save backups cannot create a new server</div>
            </div>
          )}
        </div>
